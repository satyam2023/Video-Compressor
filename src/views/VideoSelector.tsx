...
import { Movefile, compressed } from '../utils/Compressor';
...
const Video = () => {
  ...
  const pickVideo = async () => {
    try {
      ...
      console.log("Selected details::", res);
    } catch (err) {
  ...
  function handlecompress() {
    if (!compressSelect) {
      ...
      compressed(videoUri[0].fileCopyUri, CompressedUrl);
      console.log("Uri Details inside Video::", videoUri[0].name)
    }
    else if (compressSelect && compressedVideoUri) {
      Movefile(compressedVideoUri, videoUri[0].name);
